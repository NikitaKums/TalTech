...
export class Index {
  ...
  private toppingsOnPizza: IToppingOnPizza[] = [];
  @bindable private search: string = '';

  constructor(private toppingsOnPizzaService: ToppingsOnPizzaService) {
  ...
  loadData(){
    this.toppingsOnPizzaService.fetchAll('?search=' + this.search).then(
      jsonData => {
        this.toppingsOnPizza = jsonData;
      }
    );
  }
