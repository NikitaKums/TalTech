...
export class Index {
  ...
  private drinksInOrder: IDrinkInOrder[] = [];
  @bindable private search: string = '';

  constructor(private drinksInOrderService: DrinksInOrderService) {
  ...
  loadData(){
    this.drinksInOrderService.fetchAll('?search=' + this.search).then(
      jsonData => {
        this.drinksInOrder = jsonData;
      }
    );
  }
