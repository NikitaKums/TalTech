...
export class Index {
  ...
  // ============ View Methods ================
  submit(): void {
    log.debug('submit', this.productReturned);

    this.productsReturnedService.delete(this.productReturned.id).then(
      response => {
        if (response.status == 200) {
          this.router.navigateToRoute("returnsDetails", {id: this.productReturned.returnId});
        } else {
  ...
  activate(params: any, routerConfig: RouteConfig, navigationInstruction: NavigationInstruction) {
    log.debug('activate');
    this.productsReturnedService.fetch(params.id)
      .then(
        jsonData => {
