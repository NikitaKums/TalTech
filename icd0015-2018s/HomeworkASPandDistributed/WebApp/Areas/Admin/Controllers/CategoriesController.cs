...
namespace WebApp.Areas.Admin.Controllers
{
    ...
    public class CategoriesController : Controller
    {
        ...
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
            ...
            var category = await _bll.Categories.AllAsync(sortOrder, searchString, pageNumber ?? 1, 10);
            
            return View(PaginatedList<BLL.App.DTO.DomainLikeDTO.Category>.Create(category, pageNumber ?? 1, 10, 
                await _bll.Categories.CountDataAmount(searchString)));
        }
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            {
                Category = category,
                ShopSelectList = new SelectList(await _bll.Shops.AllAsync(), nameof(Shop.Id), nameof(Shop.ShopName), category.ShopId)
            };
            return View(vm);
        }
        ...
        public async Task<IActionResult> Edit(int id, CategoryCreateViewModel vm)
        {
            ...
            vm.ShopSelectList =  new SelectList(await _bll.Shops.AllAsync(), nameof(Shop.Id), nameof(Shop.ShopName), vm.Category.ShopId);
            return View(vm);
        }
