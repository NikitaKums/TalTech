...
namespace WebApp.Areas.ShopManager.Controllers
{
    ...
    public class DefectsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
            ...
            var defect = await _bll.Defects.AllAsyncByShop(User.GetShopId(), sortOrder, searchString, pageNumber ?? 1, 10);
            
            return View(PaginatedList<BLL.App.DTO.DomainLikeDTO.Defect>.Create(defect, pageNumber ?? 1, 10, 
                await _bll.Defects.CountDataAmount(User.GetShopId(), searchString)));
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            {
                Defect = defect,
                ShopSelectList = new SelectList(await _bll.Shops.GetShopByUserShopIdForDropDown(User.GetShopId()), nameof(Shop.Id), nameof(Shop.ShopName), nameof(defect.ShopId))
