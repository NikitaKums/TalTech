   ...
   public static void binaryInsertionSort(int[] a) {
      ...
      for (int i = 1; i < a.length; i++) {
         ...
         while (left < right) {
            ...
            if (arrayElement >= a[middle]) {
               left = middle + 1; //left pointer shifts to the right
            } else if (arrayElement < a[middle]) {
               right = middle; //right pointer shifts to the left
            }
         }//while


         //'left' shows the index where the value needs to be inserted, i shows the spot of the value in original array.
         //left cannot be bigger than i. if left == i then the value does not need to be replaced.
         if (left < i) {

             /*Array elements that are on and after the index that needs to be made free are shifted on to the value
             that needs to be replaced*/
            System.arraycopy(a, left, a, left + 1,  i - left);

            //value is added to it's ordered place
            a[left] = arrayElement;
            }
